...
function fn_comment_delete(method, pathname, params, cb) {
    ...
    if (params.comment_id == null) {
        ...
        var pg_query =`delete from comment where comment_id = $1`;
        console.log ("Query===>",pg_query);

        pool.query(pg_query, [params.comment_id]
            , (error, results) => {
                if (error) {
                    response.errorcode = 1;
                    response.errormessage = error;
...
function fn_sub_comment_delete(method, pathname, params, cb) {
    ...
    console.log("sub_comment_id ==>",params.sub_comment_id);
    ...
    if (params.sub_comment_id == null) {
        ...
        var pg_query =`delete from sub_comment where sub_comment_id = $1`;
        console.log ("Query===>",pg_query);

        pool.query(pg_query, [params.sub_comment_id]
            , (error, results) => {
                if (error) {
                    response.errorcode = 1;
                    response.errormessage = error;
